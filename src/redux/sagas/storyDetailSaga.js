...
function* getIndividualStory(action) {
  try {
    ...
    const serverResponse = yield axios.get(
      `story/detail/summary/${action.payload}`
    );
...
function* editChapter(action) {
  try {
    ...
    yield put({
      ...
      payload: response.data[0].story_id
    });
  } catch (e) {
...
function* getStoryContributors(action) {
  try {
    ...
    const serverResponse = yield axios.get(
      `/story/detail/contributor/${action.payload}`
    );

    yield put({
      type: "SET_STORY_DETAIL_CONTRIBUTOR",
...
function* getStoryLikes(action) {
  try {
    const serverResponse = yield axios.get(
      `/story/detail/likes/${action.payload}`,
      action.payload
    );
