...
function* storyTemplate(action) {
  try {
    ...
    const action = {
      type: "SET_TEMPLATE_STORY_DROPDOWN",
...
function* storyTemplateDetails(action) {
  try {
    ...
    const nextAction = {
      type: "SET_TEMPLATE_NEW_STORY",
      payload: response.data
    };
    yield put(nextAction);
    // get chapter details for a story
    const chapterResponse = yield axios.get(
      `/template/chapter/${action.payload}`
    );
    // set the chapter details
    const chapterAction = {
      type: "SET_TEMPLATE_NEW_CHAPTER",
      payload: chapterResponse.data
    };
